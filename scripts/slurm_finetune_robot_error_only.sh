#!/bin/bash
#SBATCH --job-name=finetune_robot
#SBATCH --output=logs/finetune_robot_%j.out
#SBATCH --error=logs/finetune_robot_%j.err
#SBATCH --time=2:00:00
#SBATCH --ntasks=1
...
# Finetune MOMENT: Progressive Unfreezing & Slicing (Error Only)
$PYTHON_EXEC -u scripts/finetune_robot_moment_error_only.py \
    --epochs 200 \
    --lr 1e-3 \
    --patience 30 \
    --batch_size 16 \
