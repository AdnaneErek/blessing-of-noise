...
def build_parser():
    parser = ArgumentParser(description="Finetune MOMENT with Random Window Slicing, Progressive Unfreezing, and Head Dropout.")
    ...
    parser.add_argument("--pretrained_path", type=str, default="checkpoints/moment_robot_pretrained.pt")
    ...
    parser.add_argument("--head_dropout", type=float, default=0.4,
                        help="Dropout probability on the classification head (default: 0.4).")
...
def main():
    ...
    model_config = Namespace(
        ...
        head_dropout=args.head_dropout,  # <-- Higher dropout for regularization
    )
    print(f"[INFO] Head dropout: {args.head_dropout}")
    model = MOMENT(model_config).to(device)

    pretrained_path = args.pretrained_path
    if os.path.exists(pretrained_path):
        state_dict = torch.load(pretrained_path, map_location=device)
        # For our new script, the state_dict might be nested under 'model_state_dict'
    ...
    checkpoint_path = os.path.join(CHECKPOINT_DIR, "moment_robot_finetuned_dropout.pt")
